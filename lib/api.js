...
async function fetchFromSource(endpointUrl) {
  try {
    const res = await fetchWithTimeout(endpointUrl, { timeout: 1500 });
    if (!res.ok) return [];
...
export async function getPostsByCategory(categorySlug, count = 12) {
  try {
    ...
    const catRes = await fetchWithTimeout(`${BASE_URL_2}/categories?slug=${slug}`, { timeout: 1500 });
    ...
    if (!catId) {
      try {
        const allCatsRes = await fetchWithTimeout(`${BASE_URL_2}/categories?per_page=100`, { timeout: 1500 });
        if (allCatsRes.ok) {
