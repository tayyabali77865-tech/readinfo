 ...
 */
function processAndMergePosts(postsArray1, postsArray2) {
  const allRaw = [...postsArray1, ...postsArray2];
  const normalized = allRaw.map(normalizePost);
...
export async function getAllPosts(page = 1, perPage = 12) {
  ...
  const posts = processAndMergePosts(data1, data2);

  if (posts.length === 0) {
    return { posts: getFallbackPosts(), totalPages: 1 };
  }
...
export async function getLatestPosts(count = 5) {
  ...
  const posts = processAndMergePosts(data1, data2).slice(0, count);
  if (posts.length === 0) {
