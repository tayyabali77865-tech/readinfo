...
async function fetchFromSource(endpointUrl) {
  try {
    const res = await fetchWithTimeout(endpointUrl, { timeout: 10000 });
    if (!res.ok) return [];
...
export async function getPostsByCategory(categorySlug, count = 12) {
  try {
    ...
    const catRes = await fetchWithTimeout(`${BASE_URL_2}/categories?slug=${slug}`, { timeout: 10000 });
    ...
    // If we could not fetch by direct slug, look up categories list to see if a fuzzy match exists
    if (!catId) {
      try {
        const allCatsRes = await fetchWithTimeout(`${BASE_URL_2}/categories?per_page=100`, { timeout: 10000 });
        if (allCatsRes.ok) {
          const allCats = await allCatsRes.json();
          const matchedCat = allCats.find(c => c.slug.includes(slug) || slug.includes(c.slug) || c.name.toLowerCase().includes(slug));
          if (matchedCat) {
            catId = matchedCat.id;
          }
        }
      } catch (e) {
        console.error("Categories lookup timed out", e.message);
      }
    }
    
    // Fallback: If no category ID is matched, query posts directly via search parameter
    if (!catId) {
      const postsData = await fetchFromSource(`${BASE_URL_2}/posts?search=${slug}&per_page=${count}&_embed=wp:featuredmedia,wp:term&status=publish`);
      return postsData.map(p => normalizePost(p, { includeContent: false }));
    }
    
    // Normal Flow: Fetch posts using the matched Category ID
...
/**
 * Fetch posts for Google News Sitemap (includes title + featured image)
 */
export async function getAllPostsForNewsSitemap() {
  const [data1, data2] = await Promise.all([
    fetchFromSource(
      `${BASE_URL_1}/posts?per_page=50&_fields=slug,date,modified,title&_embed=wp:featuredmedia&status=publish`
    ),
    fetchFromSource(
      `${BASE_URL_2}/posts?per_page=50&_fields=slug,date,modified,title&_embed=wp:featuredmedia&status=publish`
    ),
  ]);

  const all = [...data1, ...data2].map((p) => {
    ...
    return {
      ...
      date: p.date,
      modified: p.modified,
      title: decodeHtmlEntities(p.title?.rendered || p.slug),
      imageUrl,
    };
  });
  ...
  return unique.length > 0
    ? unique
    : getFallbackPosts().map((p) => ({ slug: p.slug, date: p.date, modified: p.date, title: p.title, imageUrl: p.imageUrl }));
}
